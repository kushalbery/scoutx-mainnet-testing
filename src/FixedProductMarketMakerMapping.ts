...
export function handleFeesSet(event: FeesSet): void {
  ...
  const feesSetEntity = new FeesSetInfo(event.transaction.hash.toHexString());
  feesSetEntity.newFees = event.params.newFees;
  feesSetEntity.currentLongTokenPrice = event.params.currentLongPrice;
  feesSetEntity.currentShortTokenPrice = event.params.currentShortPrice;
  feesSetEntity.timestamp = event.params.timestamp;
  feesSetEntity.questionId = event.params.questionId;
  feesSetEntity.poolBalances = event.params.poolBalances;
  feesSetEntity.feePoolWeight = event.params.feePoolWeight;
  feesSetEntity.save();
}
