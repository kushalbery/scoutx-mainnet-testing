...
export class FeesSetInfo extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));

    this.set("newFees", Value.fromBigInt(BigInt.zero()));
    this.set("currentLongTokenPrice", Value.fromBigInt(BigInt.zero()));
    this.set("currentShortTokenPrice", Value.fromBigInt(BigInt.zero()));
    this.set("timestamp", Value.fromBigInt(BigInt.zero()));
    this.set("questionId", Value.fromBytes(Bytes.empty()));
    this.set("poolBalances", Value.fromBigIntArray(new Array(0)));
    this.set("feePoolWeight", Value.fromBigInt(BigInt.zero()));
  }

  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save FeesSetInfo entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        "Cannot save FeesSetInfo entity with non-string ID. " +
          'Considering using .toHex() to convert the "id" to a string.'
      );
      store.set("FeesSetInfo", id.toString(), this);
    }
  }

  static load(id: string): FeesSetInfo | null {
    return changetype<FeesSetInfo | null>(store.get("FeesSetInfo", id));
  }
  ...
  get newFees(): BigInt {
    let value = this.get("newFees");
    return value!.toBigInt();
  }

  set newFees(value: BigInt) {
    this.set("newFees", Value.fromBigInt(value));
  }
  ...
  get feePoolWeight(): BigInt {
    let value = this.get("feePoolWeight");
    return value!.toBigInt();
  }

  set feePoolWeight(value: BigInt) {
    this.set("feePoolWeight", Value.fromBigInt(value));
  }
}
